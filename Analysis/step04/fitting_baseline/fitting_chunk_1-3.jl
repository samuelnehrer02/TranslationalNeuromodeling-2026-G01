##########################################################################################
#  STEP 04 — BASELINE FITTING: CHUNKS 1–3
#  ─────────────────────────────────────────────────────────────────────────────────────
#  Run on WORKSTATION (20 workers) from the project root:
#
#      julia --project=. "Analysis\step04\fitting_baseline\fitting_chunk_1-3.jl"
#
#  Input:  fitting_baseline/baseline_chunk_1_3.jld2  (key: "df_baseline")
#  Output: fitting_baseline/model_N/participant_<ID>.jld2
#          (model_1–model_4 folders already exist; chains saved directly inside)
#
#  · 20 workers, one job per (model × participant), shuffled for load balance.
...
@everywhere begin
    ...
    function fit_with_retry(
        ...
        data_path = joinpath(@__DIR__, "baseline_chunk_1_3.jld2")
...
##########################################################################################
df_chunk   = load(joinpath(@__DIR__, "baseline_chunk_1_3.jld2"), "df_baseline")
chunk_pids = unique(df_chunk.ID)

jobs = [(model_id = m, participant_id = p) for m in 1:4 for p in chunk_pids]
shuffle!(jobs)

@info "Chunks 1–3: $(length(jobs)) jobs (4 models × $(length(chunk_pids)) participants), 20 workers…"
t_start = time()
...
println("\n" * "─"^60)
println("  CHUNKS 1–3 SUMMARY  ($(elapsed)s total wall time)")
println("─"^60)
